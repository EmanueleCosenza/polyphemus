...
class VAETrainer:
    ...
    def train(self, trainloader, validloader=None, epochs=1, early_exit=None):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, inputs in enumerate(trainloader):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    tot_loss, losses = self._compute_losses(
                ...
                if (self.tot_batches + 1) % self.print_every == 0:
                    print(
                        "Training on batch {}/{} of epoch {}/{} complete.".format(
                            batch_idx + 1, len(trainloader), epoch + 1, epochs
                        )
                    )
                    self._print_stats()
                    print("\n----------------------------------------\n")
                ...
                if (
                    ...
                    and (self.tot_batches + 1) % self.eval_every == 0
                ):
                    # Evaluate on val set
                ...
                if (
                    self.save_every > 0
                    and (self.tot_batches + 1) % self.save_every == 0
                ):
        ...
        print(
            "Training completed in (h:m:s): {:0>2}:{:0>2}:{:05.2f}".format(
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(loader):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    _, losses_b = self._compute_losses(inputs, outputs, mu, log_var)
    ...
    def _compute_accuracies(self, inputs, outputs, is_drum):
        ...
        pitches_acc_drums = self._pitches_accuracy(seq_rec, x_seq, is_drum, drums=True)
        pitches_acc_non_drums = self._pitches_accuracy(
            seq_rec, x_seq, is_drum, drums=False
        )
    ...
    def _note_accuracy(self, seq_rec, x_seq):
        ...
        return torch.sum(torch.logical_and(preds_pitches, preds_dur)) / torch.sum(
            mask_p
        )
    ...
    def _print_stats(self):
        ...
        minutes, seconds = divmod(rem, 60)
        print(
            "Elapsed time from start (h:m:s): {:0>2}:{:0>2}:{:05.2f}".format(
                int(hours), int(minutes), seconds
            )
        )
